...
void funcion_hebra_productora( ProdCons2SC * monitor, int n, int index )
{
   int p = num_items/n;
   for( unsigned i = index*p ; i < (index*p + p) ; i++ )
   {
      int valor = producir_dato(index) ;
      monitor->escribir( valor );
   }
}
// -----------------------------------------------------------------------------

void funcion_hebra_consumidora( ProdCons2SC * monitor, int n, int index)
{
   int c = num_items/n;
   for( unsigned i = 0 ; i < c ; i++ )
   {
      int valor = monitor->leer();
      consumir_dato( valor, index ) ;
   }
}
// -----------------------------------------------------------------------------
...
int main()
{
   ...
   int n_prod = 10, n_cons = 10;
   ProdCons2SC monitor ;

   thread hebra_productora[n_prod],
          hebra_consumidora[n_cons];

   for(int i = 0; i < n_prod; i++)
     hebra_productora[i] = thread( funcion_hebra_productora, &monitor, n_prod, i);

   for(int i = 0; i < n_cons; i++)
     hebra_consumidora[i] = thread( funcion_hebra_consumidora, &monitor, n_cons, i);
